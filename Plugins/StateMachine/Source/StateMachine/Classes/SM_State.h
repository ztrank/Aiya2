...
enum class EStateMachineCompletionType : uint8
{
	...
	// Out simulation ran out of steps while the machine was still running.
...
public:
	/* Returns DestinationState on success, null on failure, for subclasses,
	 OutDataIndex might be somethign other than 1, if a branch is made to consume multiple inputs */
...
protected:
	...
	// Accetable Inputs. The current input atoms must be on this list.
	UPROPERTY(EditAnywhere)
...
protected:
	...
	/** If this is set, this state will loop on itself
	whenever an unhandled input atom is detected */
	UPROPERTY(EditAnywhere)
	uint32 bLoopByDefault : 1;

	/** Branches to other states. These are in priority order, 
	so the first successful branch will be taken */
	UPROPERTY(EditAnywhere, Instanced)
	TArray<USM_Branch*> InstancedBranches;

	/* Branches to other states. These are in priority order,
	so the first successful branch will be taken. These run after InstancedBranches*/
	UPROPERTY(EditAnywhere)
